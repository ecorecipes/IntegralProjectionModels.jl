...
In density-dependent (DD) IPMs, vital rates depend on the current
population size $N_t = \int n(z, t) \, dz$. The kernel $K(z', z; N_t)$
is rebuilt at each time step, so the population dynamics are nonlinear.

This vignette models **Soay sheep** with density-dependent survival,
recruitment probability, and recruit size, following Ellner, Childs &
Rees (Chapter 5). The key goal is to find the **equilibrium population
size** $\bar{N}$ where $\lambda(N) = 1$.
...
Density-dependent parameters have an additional `Nt` term that reduces
the vital rate as population size increases.
...
![](05_density_dependent_files/figure-commonmark/cell-6-output-1.svg)

## Building the DD Kernel

For a DD model, the kernel is a function of the current population
state. We pass this function to `IPMProblem` with the
`DensityDependent()` tag.
...
```
    ...
    dd_kernel (generic function with 1 method)

## Lambda as a Function of Population Size

At the equilibrium population size $\bar{N}$, the asymptotic growth rate
$\lambda(\bar{N}) = 1$. We can compute $\lambda(N)$ for a range of $N$
values.
...
![](05_density_dependent_files/figure-commonmark/cell-8-output-1.svg)

## Finding the Equilibrium

The equilibrium $\bar{N}$ is the population size where $\lambda(N) = 1$.
We find it using bisection.
...
```
    ...
    Equilibrium population: N̄ = 322.7 females
    λ at equilibrium: 0.999999

The equilibrium is approximately **323 females**, matching the R result
from `uniroot`.

## Population Trajectories

We can iterate the DD-IPM from different starting population sizes to
see convergence to the equilibrium.
...
![](05_density_dependent_files/figure-commonmark/cell-11-output-1.svg)

All trajectories converge to the same equilibrium, regardless of
starting population size.

## Using IPMProblem with DensityDependent()

We can also use the `IPMProblem` interface with the `DensityDependent()`
tag.
...
![](05_density_dependent_files/figure-commonmark/cell-12-output-1.svg)
...
![](05_density_dependent_files/figure-commonmark/cell-13-output-1.svg)

## Equilibrium Size Distribution

``` julia
# Iterate to convergence from equilibrium N
n_eq = fill(N_eq / (m * h), m)
for _ in 1:50
...
![](05_density_dependent_files/figure-commonmark/cell-14-output-1.svg)

## Summary

In this vignette we:

1.  Built a density-dependent IPM where survival, recruitment, and
    recruit size decrease with population size
2.  Computed $\lambda(N)$ across a range of population sizes
3.  Found the equilibrium population $\bar{N} \approx 323$ females where
    $\lambda = 1$
4.  Demonstrated convergence from multiple starting conditions
5.  Used both manual iteration and the
